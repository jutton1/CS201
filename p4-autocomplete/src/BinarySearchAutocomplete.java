...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		if (first == -1 || k == 0 || last>=myTerms.length) {               // prefix not found
			return new ArrayList<>();
		}

		// write code here for P5 assignment
		PriorityQueue<Term> mySortedTerms = new PriorityQueue<Term>(Comparator.comparing(Term::getWeight));
		for (int i = first; i < last + 1; i ++) {
			if (mySortedTerms.size()<k) {
				mySortedTerms.add(myTerms[i]);
				continue;
			}
			if (myTerms[i].getWeight() > mySortedTerms.peek().getWeight()) {
				mySortedTerms.remove();
				mySortedTerms.add(myTerms[i]);
			}
		}

		LinkedList<Term> returnList = new LinkedList<Term>();
		// just in case mySortedTerms.size() is ran multiple times in the code
		int returnSize = Math.min(mySortedTerms.size(),k);
		
		for (int i = 0; i < returnSize; i++) {
			returnList.addFirst(mySortedTerms.remove());
