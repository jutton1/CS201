...
public class    PrefixComparator implements Comparator<Term> {
    ...
    public int compare(Term v, Term w) {
        ...
        char[] wordV = v.getWord().toCharArray();
        char[] wordW = w.getWord().toCharArray();

        

        for (int i = 0; i < myPrefixSize; i++) {
            if (wordV.length == i || wordW.length == i) {
                return wordV.length - wordW.length;
            }
            if (wordV[i] != wordW[i]) {
                return wordV[i] - wordW[i];
            }
        }
        // Considered equal if they run through the loop and are
        // equal at all char up to myPrefixSize
        return 0;
    }
}
